...
# always use the better annotated gene for target 1
...
awk '$7 == "CDS"' > Working/${GENOME_TARGET_1}_coding.bed

./tbl2bed.awk ${GENOME_TARGET_2}.tbl |
awk '$7 == "CDS"'> Working/${GENOME_TARGET_2}_coding.bed

# convert the bedfiles into fasta nucleotide sequences
bedtools getfasta -nameOnly -s \
  -bed Working/${GENOME_TARGET_1}_coding.bed \
  -fi ${GENOME_TARGET_1}.fasta |
sed 's/(.)//g'> Working/${GENOME_TARGET_1}_coding.fna

bedtools getfasta -nameOnly -s \
  -bed Working/${GENOME_TARGET_2}_coding.bed \
  -fi ${GENOME_TARGET_2}.fasta |
sed 's/(.)//g' > Working/${GENOME_TARGET_2}_coding.fna

# some nucleotide sequences are discontiguous, so merge them
awk 'FNR%2==1 && !($0 in seen){print; seen[$0]; getline; print} FNR%2==1 && $0 in seen {getline; print}' \
  Working/${GENOME_TARGET_1}_coding.fna |
  awk 'BEGIN{ORS=""} /^>/ { $0 = (NR==1 ? "" : RS) $0 RS } END { printf RS }1' > Working/tmp &&
  mv Working/tmp Working/${GENOME_TARGET_1}_coding.fna

awk 'FNR%2==1 && !($0 in seen){print; seen[$0]; getline; print} FNR%2==1 && $0 in seen {getline; print}' \
  Working/${GENOME_TARGET_2}_coding.fna |
  awk 'BEGIN{ORS=""} /^>/ { $0 = (NR==1 ? "" : RS) $0 RS } END { printf RS }1' > Working/tmp &&
  mv Working/tmp Working/${GENOME_TARGET_2}_coding.fna

# translate them into amino acid sequences
seqkit translate -f1 -x -w0 -T11 -x -M \
  Working/${GENOME_TARGET_1}_coding.fna > Working/${GENOME_TARGET_1}_coding.faa

seqkit translate -f1 -x -w0 -T11 -x -M \
  Working/${GENOME_TARGET_2}_coding.fna > Working/${GENOME_TARGET_2}_coding.faa

# use exonerate to generate a list of pair-wise best matches
# output the query, target, percent similarity, raw score
exonerate \
  --bestn 1 \
  -q Working/${GENOME_TARGET_1}_coding.faa \
  -t Working/${GENOME_TARGET_2}_coding.faa \
  --showalignment 0 \
  --showvulgar 0 \
  --ryo "%qi\t%ti\t%ps\t%s\t"\#ortho"\n" > Working/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos.tsv

exonerate \
  --bestn 1 \
  -q Working/${GENOME_TARGET_2}_coding.faa \
  -t Working/${GENOME_TARGET_1}_coding.faa \
  --showalignment 0 \
  --showvulgar 0 \
  --ryo "%ti\t%qi\t%ps\t%s\t"\#ortho"\n" >> Working/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos.tsv
...
awk '$5=="#ortho" {seen[$1 OFS $2 OFS $3 OFS $4]++} END{for(i in seen){if(seen[i]>1){print i}}}' \
  Working/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos.tsv > Working/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos_best.tsv

# retrieve gene extended gene information
esearch -db gene -query "$GENOME_TARGET_1\[NUCL_ACCN\]" |
  efetch -format tabular |
  awk 'FNR==1{print $3, $6, $7; seen[$3]} !($3 in seen) {seen[$3]; split($7, syn, ", "); for(i in syn) {print $3, $6, syn[i], $5;}}' > Working/${GENOME_TARGET_1}_genes.tsv
...
# add gene names
awk 'NR == FNR {def[$3]=$1 OFS $2} NR != FNR && $1 in def{print $0, def[$1]}' Working/${GENOME_TARGET_1}_genes.tsv Working/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos_best.tsv  | sort -k6,6V > Working/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos_best_ext.tsv

# only use gene namess
awk '{print $6}' Working/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos_best_ext.tsv > Enrichment/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos_sym.tsv
...
awk 'NR==FNR {ess[$1]=$NF} NR != FNR {print $0, ess[$1]}' \
  ${GENOME_TARGET_1}_transit_tn5gaps.tsv Working/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos_best_ext.tsv |
...
# number of orthologs
BACKGROUND="$(wc -l Working/${GENOME_TARGET_1}_${GENOME_TARGET_2}_orthos_best_ext.tsv |
  cut -d' ' -f1)"
